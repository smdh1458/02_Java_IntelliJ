...
public class FileService {
    ...
    public void method1(){
        ...
        //폴더 절대경로C:\newFolder
        //현재 우리가 실행하고 있는 파일 위치기준 11_io 폴더 안에서 User/user1/Desktop/newFolders 만든다는 것
    ...
    public void method2(){
        ...
        if (file.exists()) {
            ...
            try {//폴더만들기와 다르게 파일 만들기는
                //문제가 생길 가능성이 높기 때문에 try - catch 나 thorow 와 같은 에러방지 구문을 추가해줘야함
                file.createNewFile();
            } catch (IOException e) {
                System.out.println("파일을 만드는데 문제가 발생했습니다.");
            }
            System.out.println("파일이 성공적으로 생성 되었습니다.");
        }
    }
    ...
    public void method3(){
        ...
        //향상된 for문을 이용해서 출력
        for (File f : files) {// 일반 for문이 아니라 향상된 for문
            System.out.println("파일 이름: "+f.getName());// 파일 이름들을 가져올 수 있음
            System.out.println("파일 크기: "+f.length()+"바이트");//.length -> 파일크기
            System.out.println("마지막 수정 날짜: "+f.lastModified());
            //마지막 수정 날짜: 1730075603084 -> 1970년 1월 1일부터 지난 일자
            ...
            //날짜 형식을 문자열 형태로 변환
            String formatDate = sdf.format(date); //1730075603084 -> sdf형식으로 변경해서 문자열로 전달
            //Cannot format given Object as a Date 파일의 마지막 수정 날짜를 최대한 큰 공간으로 가져온 뒤에 Date로 변환
