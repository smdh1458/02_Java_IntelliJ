...
public class FileReadWrite {
    ...
    public void method1(){
        System.getProperty("user.home"); //C/users/user1 + 바탕화면
                            //      C       users   user1   바탕화면에서 ex1.txt 만들기
        ...
        //생성된 파일 글자를 작성하기
        //글자를 작성할 때 BufferdWriter FileWriter를 사용해서 작성
        try {//파일 안에 글자를 작성하는 건 문제가 많을 수 있기 때문 예외상황 감지하는 try - catch 사용
            FileWriter writer = new FileWriter(file); //File안에 Writer글자 작성
    ...
    public void method2(){
        ...
        try {
            ...
            String content2 = "\n이어서 작성한 내용입니다."; //새로운 내용을 추가
            fw.write(content2);
            fw.close();//이어서 작성 완료 후 파일닫기 = 저장
    ...
    // BufferedReader : FileReader에 버퍼링 기능 추가한 클래스          -> 한 줄단위로 읽음
    ...
    public void method3(){
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(file);//한줄 단위로 읽는 성능좋은 부스터 붙임
    ...
    public void method4(){
        ...
        try {
            ...
            while ((line = br.readLine()) != null) { //line이라는 공간에 한줄의 글자들이 담기고
                System.out.println(line);
                //line이라는 변수명에 담겨진 글자를 출력한 다음 다음줄이 아예 없을 때까지 계속해서 출력을 진행함
            }
            //복사기에서 5페이지를 추력하기 위해 1페이지 출력하고 2페이지 뽑을 준비하는거랑 같음
            br.close();
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
