 ...
 * com c.o.m으로 구분지어서 작성 X
 * c.o.m -> c라는 폴더 안에 o라는 폴더안에 m이라는 폴더안에 만들겠다는 표기
 *
 * */
public class VariableEx1 {
    ...
    public static void main(String[] args) {
        ...
        //보라색으로 바뀌는 언어는 예약어
        boolean 진실혹은거짓 = true;//진실 혹은 거짓 이라는 변수명에 1byte true가 저장
        System.out.println("진실 혹은 거짓은 => "+ 진실혹은거짓);

        double 실수1번;	//메모리에 double 저장공간 8byte 할당하고 그 공간을
        // 실수 1번으로 부르겠다.
        ...
        String 문자열2번 = "필수로 맨 앞 S는 대분자 표기 보라색으로 글자 변화X";
         ...
         * 예약어(자료형, 참조형) 변수명 = 리터럴;
         * [정수형]
         * 정수형 기본적으로 리터널이 int로 설정
         * int(4byte) 안에서 int보다 byte 크기가 작은 short나 byte 로 리터널 변경할 경우
         ...
         * int는 위에서 작성한 숫자 범위를 초과하기 때문에 초과하는 숫자를 담기 위해서는 뒤에 L이나 l을 붙여야함
         *
         * long 변수명 = long
         * long	롱2번 = 10000000000L; //long 사용 long 리터널은 L필수로 뒤에 와야함
         *
         * [실수형]
         * 실수형 리터널 기본적으로 double 타입으로 설정됨
         * 왜냐하면 float은 대략 소수점아래 7번째자리까지만 표기 가능
         *
         * 나머지 8자리부터 버리기 때문에 안전하게 가져울수 있도록 double 기본으로 사용
         * 소수점 아래 소수들을 나타낼 때 8자리 이상이 아닐 경우
         ...
         * 실수는 기본리터널형인 double로 인식이 되어
         * 공간보다 큰 메모리가 온다고 컴퓨터는 인식을 하고 오류를 발생하기 때문에
         * 앞에 예약어를 float을 작성했다면 소수 뒤에는 f를 붙여주는 것
         *
         * L이나 F같은 리터널 표기법은
