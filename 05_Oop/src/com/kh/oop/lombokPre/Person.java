...
//Alt Shift Enter -> 가져와야하는 경로가 있다면 자동 추가
/*
lombok 을 사용해기 위해 종속성 설정을 진행해줘야함
 하지만 현재는 진행을 할 수 없음
*/
...
public class Person {
    ...
    /*
    import lombok.NoArgsConstructor; -> 롬북 회사에서 만든 기본생성자 기능 가져오기~~
    @NoArgsConstructor               -> 기능 가져와서 사용하기

    기본생성자   = 넣어야하는 매개변수가 없기 때문에 NoArgsConstructor
    () -> 넣어야하는 값이 없기 때문에 매개변수명도 존재하지 않음 그래서 NoArgs!
    public Person(){
    ...
    */
    ...
    아래 코드 대신에 위 두줄을 이용해서 매개변수 값이 들어있는 생성자 표기 가능
