...
public class HashSetPre1 {
    ...
    */
    ...
    public void method1(){
        HashSet set = new HashSet(); // 자료형 제한없이 String int, 개발자가 만든 자료형 등.... 모두 OK
        set.add("점심");
        set.add(100);
        set.add(true);
        set.add(3.14);
    ...
    public void method2(){
        ...
        //바나나가 set안에 들어있는지 확인
        set.contains("바나나"); //결과가 들어있다면 true 이고 없다면 false
        ...
        System.out.println("제대로 모두 비워짐?: "+set.isEmpty());
    ...
    public void method3() {
        ...
        intSet.add(500);
        intSet.add(40);
        System.out.println("내용확인 :"+intSet);
        intSet.remove(3000);
        System.out.println("삭제확인 :"+intSet);
        System.out.println("500있음? : "+intSet.contains(500));
        intSet.clear();
        System.out.println("다 지움? : "+intSet);
    }

    // set 을 향상된 for 문 (= for-each) 출력하기
    // 일반 for 문 경우 index 가 존재하지 않기 때문에 사용 제한
    // 대신 Iterator 라는 반복문을 이용해서 출력
    public void method4() {
    ...
    public void method5() {
        ...
        //HashSet    목록을 담고있는 str변수명을 가져와서 반복하겠다 설정
        ...
        *            (HashSet 이외에도 ArrayList 등 Collection 으로 작성된 객체들에게 모두 사용 가능)
        *
        ...
        *                     false = 더이상 반복할 요소(=리스트 목록 안에 들어가있는 값)가 없음
        *
    ...
    public void iterLotto() {
        ...
        //2. 중복없이 6개의 번호 생성
        //로또숫자들이 총 0부터 5까지 6개 이하일 때만 for 문을 반복한다

            for(int i = 0;lottoNumbers.size() < 6; i++) {
                //어떤 숫자를 추가할거냐면 Math.random 으로 가져온 숫자를 사용
                // Math.random() = 0.0 ~ 1.0 사이의 소수를 출력 0.00000001 ~ 0.9999999999 사이 소수 출력
                // Math.random() * 45.2312132465132 앞에 (int) 를 붙여주면 .뒤에 있는 소수들이 버려짐
                ...
                lottoNumbers.add(num);// 만들어진 숫자를 중복 허용되지 않는 set담기
            }
            ...
            while(it.hasNext()) {
                System.out.println(it.next()); //로또 번호들이 출력됨
            }

    }

    public void foriterLotto() {
        //1. HashSet을 이용해서 로또 번호 저장
        HashSet<Integer>lottoNumbers = new HashSet<>();
