...
public class ArrayEx {
    ...
    * - 주소를 저장하는 변수
    *
    * -변수 사용시 저장된 주소로 찾아가서 그 곳에 위치한 배열, 객체를 참조
    *
    * -기본 자료형 8개를 제외한 나머지는 모두 참조형 변수
    * * 기본 자료형 = byte , short, int, long, chr, boolean, float, double
    * 대표적인 참조 자료형 = String, Scanner, ..이외 우리가 만든 자료형이 될 수 있음
    *
    * 자료형이 맨 앞글자가 소문자로 시작하면 기본 자료형
    * 자료형이 맨 앞글자가 대문자로 시작하면 개발자가 만들어주는 참조 자료형
    *
    * */
    ...
    * */
    ...
    public void mathod1() {
       ...
       값들을 넣어 초기화한 상태
       arr[0] = 100 arr[1] = 200 arr[2] = 300 arr[5];
       ...
       초기화 후
        [Stack]                         [heap]
            ...
            arr = new int[4]실행 시
                    -heap 크키 4짜리 배열이 생성되고 초기값은 0으로 설정
                     heap 생성된 배열을 시작 주소가 Stack의 arr에 저장됨
                     이제 arr은 Heap의 배열을 참조할 수 있음
            결과적으로
                    Stack : arr변수는 Heap 배열의 주소를 가짐
                    Heap : 배열이 생성되어 초기화된 상태

                    arr[0] = 100 arr[1] = 200 arr[2] = 300 arr[5] 값을 넣었을 때
    ...
    * */
    public void mathod3() {
        ...
        //에러 발생 반드시 int나 변수명 앞 뒤에 []붙여야함
        //단 int와 같은 자료형 앞은 붙이기 X
        
        /*
        기본 자료형에 [] 붙였기 때문에 참조형 변수가 되는 것
        * int arr = new int[4]; (X) 
        * []int arr = new int[4]; (X) 
        * int[] arr = new int[4]; (O) 
        * int []arr = new int[4]; (O) 
    ...
    public void mathod4() {
        ...
        //[I@5dfcfece와 같은 주소값이 보임
        //[ 배열 시작 뜻 I는 정수형이라는 뜻 @뒤는 배열구분명칭 16진수 표기
